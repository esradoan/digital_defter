...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20251219162513_InitialCreate') THEN
...
CREATE PROCEDURE MigrationsScript()
BEGIN
    IF NOT EXISTS(SELECT 1 FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20251220123215_AddSerialNoAndMakeFieldsOptional') THEN
...
CREATE PROCEDURE MigrationsScript()
BEGIN
    IF NOT EXISTS(SELECT 1 FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260225080455_MakeUnitOptional') THEN

    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20260225080455_MakeUnitOptional', '9.0.0');
...
CREATE PROCEDURE MigrationsScript()
BEGIN
    IF NOT EXISTS(SELECT 1 FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260225080723_MakeUnitOptional2') THEN

    ALTER TABLE `Products` MODIFY COLUMN `Unit` varchar(50) CHARACTER SET utf8mb4 NULL;
...
BEGIN
    ...
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20260225080723_MakeUnitOptional2', '9.0.0');
...
CREATE PROCEDURE MigrationsScript()
BEGIN
    IF NOT EXISTS(SELECT 1 FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260228113721_AddProtocols') THEN

    CREATE TABLE `ProtocolCategories` (
        `Id` int NOT NULL AUTO_INCREMENT,
        `Name` varchar(100) CHARACTER SET utf8mb4 NOT NULL,
        `Description` varchar(500) CHARACTER SET utf8mb4 NULL,
        `CreatedAt` datetime(6) NOT NULL,
        `UpdatedAt` datetime(6) NULL,
        `CreatedBy` int NULL,
        CONSTRAINT `PK_ProtocolCategories` PRIMARY KEY (`Id`)
    ) CHARACTER SET=utf8mb4;
...
BEGIN
    ...
    CREATE TABLE `Protocols` (
        ...
        `ProtocolCategoryId` int NULL,
        `CreatedAt` datetime(6) NOT NULL,
        `UpdatedAt` datetime(6) NULL,
        `CreatedBy` int NULL,
        CONSTRAINT `PK_Protocols` PRIMARY KEY (`Id`),
        CONSTRAINT `FK_Protocols_ProtocolCategories_ProtocolCategoryId` FOREIGN KEY (`ProtocolCategoryId`) REFERENCES `ProtocolCategories` (`Id`) ON DELETE SET NULL
    ) CHARACTER SET=utf8mb4;
...
BEGIN
    ...
    CREATE INDEX `IX_Protocols_ProtocolCategoryId` ON `Protocols` (`ProtocolCategoryId`);
...
BEGIN
    ...
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20260228113721_AddProtocols', '9.0.0');
